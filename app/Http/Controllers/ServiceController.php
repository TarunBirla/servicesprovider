...
class ServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                'associate_trade_name.*' => 'required|string|max:255',
                'type_0' => 'required|string', // Ensure first section is filled
                'revenue_type.*' => 'required|string',
                'sector_name.*' => 'required|string',
                'industry_name.*' => 'required|string',
                'sub_industry_name.*' => 'required|string',
            ...
            foreach ($request->associate_trade_name as $index => $tradeName) {
                Service::create([
                    ...
                    'associate_id' => $associate_id,
                    'title' => $tradeName,
                    'type' => $request->input("type_$index") ?? null,
                    'revenue_type' => $request->revenue_type[$index] ?? null,
                    'sector_name' => $request->sector_code[$index] ?? null,
                    'industry_name' => $request->industry_code[$index] ?? null,
                    'sub_industry_name' => $request->subindustry_code[$index] ?? null,
                    'experience_year' => $request->experience_year[$index] ?? null,
                    'amount' => $request->amount[$index] ?? null,
                    'address' => $request->associate_trade_address[$index] ?? null,
                    'pincode' => $request->associate_trade_pincode[$index] ?? null,
                    'state' => $request->associate_trade_st_ut_name[$index] ?? null,
                    'district_name' => $request->associate_trade_district_name[$index] ?? null,
                    'assembly_name' => $request->associate_trade_assembly_name[$index] ?? null,
                    'part_name' => $request->associate_trade_part_name[$index] ?? null,
                ]);
            }
